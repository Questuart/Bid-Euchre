"""Review coordinator driver — canonical review coordinator for the repository.

This module is the **single reviewer of record** for all PRs.  It owns:

- The merge-relevant ``reviewing-changes`` commit status.
- The single upserted machine-owned PR summary comment
  (identified by ``<!-- review-loop-comment -->``).
- Fallback/recovery behavior when a reviewer backend is unavailable.

No other system (hosted ``claude-review``, Codex Cloud, or ad-hoc bot
comments) may publish the ``reviewing-changes`` status or claim to be the
canonical review truth.  Those surfaces are advisory overlays only.
...
# ---------------------------------------------------------------------------
# Coordinator-of-record contract
# ---------------------------------------------------------------------------
# This constant documents the architectural boundary.  Other modules may
# read it; nothing should override these declarations.

COORDINATOR_CONTRACT: dict[str, str] = {
    "status_context": "reviewing-changes",
    "status_publisher": "review_driver.py",
    "comment_marker": "<!-- review-loop-comment -->",
    "comment_publisher": "review_driver.py (via github_pr_state.upsert_review_comment)",
    "preferred_backend": "codex_cli",
    "fallback_behavior": "degraded_pass",
    "advisory_surfaces": "claude-review, codex-cloud",
    "rerun_command": "python scripts/internal/review_driver.py --pr <N> --trigger manual",
}
"""Canonical ownership declarations for the review coordinator.

- ``status_context`` / ``status_publisher``: only this module publishes
  the ``reviewing-changes`` commit status.
- ``comment_marker`` / ``comment_publisher``: only this module creates or
  updates the machine-owned PR summary comment.
- ``preferred_backend``: the primary review backend (Codex CLI).
- ``fallback_behavior``: when the backend is unavailable or returns
  unparseable output, the coordinator publishes a ``degraded`` pass
  (GitHub ``success`` with a distinct description).
- ``advisory_surfaces``: hosted reviewer surfaces that are explicitly
  **not** the reviewer of record.
- ``rerun_command``: operator-facing recovery command.
"""
...
def _create_follow_up_issues(
    ...
    for label, cat_findings in by_label.items():
        ...
        body = (
            ...
            "Findings from autonomous review coordinator that should be "
            "addressed post-merge.\n\n"
            f"### Findings\n{findings_table}\n"
            "---\n"
            f"*Auto-generated by review coordinator. Original PR: #{pr_number}*"
        )
...
def initialize_state(
    ...
    *,
    ...
    """Create initial state for a new review coordinator run.
...
def _step_initialized(
    ...
    _publish_status(loop_state.pr_number, "pending", "Review coordinator started")
...
def _format_review_comment(
    ...
    Args:
        loop_state: Current review coordinator state.
    ...
    if status == "blocked":
        header = "## \u274c Review Coordinator -- Blocked"
    else:
        header = "## \u2705 Review Coordinator -- Passed"

    lines = [
        REVIEW_COMMENT_MARKER,
        header,
        "",
        "> Canonical review summary from the local review coordinator.",
        "> This is the single machine-owned review comment of record for this PR.",
        "",
    ]
    ...
    lines.append("")
    lines.append(
        f"*Auto-generated by review coordinator (iteration "
        f"{loop_state.iteration_count}/{loop_state.max_iterations}). "
        f"Other review comments (claude-review, Codex Cloud) are advisory only.*"
    )
...
def _step_pr_open(
    ...
    # NOTE: Local `make check` was removed because the review coordinator runs in the
...
def _step_retesting(
    ...
    Local `make check` was removed — the review coordinator runs in the main checkout,
...
def main() -> int:
    ...
    if loop_state is None:
        if args.branch is None:
            logger.error("--branch required for new review coordinator runs")
            return 1
