...
def plot_win_push_loss_by_contract(ax, results: List[Dict]):
    """Panel 6: Win/Push/Loss rates by contract (stacked horizontal bars with CI)."""
    from bid_euchre.reporting import compute_outcome_stats, OUTCOME_COLORS, format_pct
    
    # Compute Win/Push/Loss for each contract
    outcomes_by_contract = {}
    for result in results:
        label = result["_label"]
        dist = get_distribution(result)
        
        # Convert distribution to trick counts list
        trick_counts = []
        for tricks, count in dist.items():
            trick_counts.extend([tricks] * count)
        
        if trick_counts:
            stats = compute_outcome_stats(trick_counts)
            outcomes_by_contract[label] = stats
    
    if not outcomes_by_contract:
        ax.text(0.5, 0.5, "No data", ha="center", va="center", transform=ax.transAxes)
        ax.set_title("Win / Push / Loss by Contract")
        return
    
    # Prepare data for stacked bar chart
    labels = list(outcomes_by_contract.keys())
    display_labels = [CONTRACT_LABELS.get(l, l) for l in labels]
    
    win_rates = [outcomes_by_contract[l].win_rate * 100 for l in labels]
    push_rates = [outcomes_by_contract[l].push_rate * 100 for l in labels]
    loss_rates = [outcomes_by_contract[l].loss_rate * 100 for l in labels]
    n_totals = [outcomes_by_contract[l].n_total for l in labels]
    
    # Sort by win rate
    sorted_data = sorted(
        zip(display_labels, win_rates, push_rates, loss_rates, n_totals, labels),
        key=lambda x: x[1]
    )
    display_labels, win_rates, push_rates, loss_rates, n_totals, labels = zip(*sorted_data)
    
    y = np.arange(len(labels))
    
    # Stacked horizontal bars
    ax.barh(y, loss_rates, color=OUTCOME_COLORS["loss"], alpha=0.85, 
            label="Loss (≤4)", edgecolor="white", linewidth=0.5)
    ax.barh(y, push_rates, left=loss_rates, color=OUTCOME_COLORS["push"], 
            alpha=0.85, label="Push (5)", edgecolor="white", linewidth=0.5)
    ax.barh(y, win_rates, left=[l+p for l,p in zip(loss_rates, push_rates)], 
            color=OUTCOME_COLORS["win"], alpha=0.85, label="Win (≥6)", 
            edgecolor="white", linewidth=0.5)
    
    # Reference line at 50%
    ax.axvline(x=50, color="#bdc3c7", linestyle="--", linewidth=1, alpha=0.5, zorder=0)
    
    ax.set_yticks(y)
    ax.set_yticklabels(display_labels, fontsize=8)
    ax.set_xlabel("Percentage (%)", fontsize=9)
    ax.set_title("Win / Push / Loss by Contract", fontsize=10, fontweight="bold")
    ax.set_xlim(0, 100)
    ax.legend(loc="lower right", fontsize=7, framealpha=0.9)
    
    # Add sample size annotations
    for i, n in enumerate(n_totals):
        ax.text(102, i, f"n={n:,}", va="center", fontsize=6, color="#7f8c8d")
...
def generate_individual_plots(
    ...
    # 6. Win/Push/Loss by Contract
    fig, ax = plt.subplots(figsize=(10, 7), facecolor="white")
    plot_win_push_loss_by_contract(ax, results)
    fig.tight_layout(pad=1.5)
    filepath = os.path.join(plots_dir, "06_win_push_loss_by_contract.png")
    fig.savefig(filepath, dpi=150, facecolor="white", edgecolor="none", bbox_inches="tight")
    plt.close(fig)
    saved_files.append(filepath)
    print(f"   ✓ {os.path.basename(filepath)}")
...
def generate_dashboard(
    ...
    plot_win_push_loss_by_contract(ax6, results)
