#!/usr/bin/env python3
"""
Analyze the distribution of tricks won on SUIT contracts for CCrider vs OLSa_Ceiling.

Usage:
    PYTHONPATH=src python experiments/analyze_ccrider_vs_ceiling.py
"""
...
def run_analysis(n_hands: int = 20000, seed: int = 42):
    ...
    print(f"Analyzing CCrider vs OLSa_Ceiling on Suit Contracts ({n_hands:,} hands, seed={seed})")
    print("-" * 80)
    
    # Define strategies
    olsa_ceiling = RegressionBidder(
    ...
    # Team 0 = OLSa_Ceiling (seats 0, 2)
    # Team 1 = OLSa_CCrider (seats 1, 3)
    strategies = [olsa_ceiling, olsa_ccrider, olsa_ceiling, olsa_ccrider]
    ...
    for i in range(n_hands):
        ...
        t0, t1, _, _, leader, starting_hands, bid, *_ = play_single_hand(
            contract_type=None,
        ...
        # Only track suit contracts for detailed analysis
        if contract_type != "suit":
            continue
        
        # Track by which team won the bid
        if winning_team == 0:
            ceiling_tricks.append(tricks_won)
            ceiling_bids.append(bid)
        else:
            ccrider_tricks.append(tricks_won)
            ccrider_bids.append(bid)
    
    # Convert to numpy arrays
    ceiling_tricks = np.array(ceiling_tricks)
    ccrider_tricks = np.array(ccrider_tricks)
    ceiling_bids = np.array(ceiling_bids)
    ...
    print()
    print(f"Point Differential: {point_diff:+,} ({'Ceiling' if point_diff > 0 else 'CCrider'} ahead)")
    
    # Print suit contract statistics
    print("\n" + "="*80)
    print("SUIT CONTRACT STATISTICS")
    print("="*80)
    print(f"\nOLSa_Ceiling (n={len(ceiling_tricks):,} suit contracts won):")
    print(f"  Tricks Won:  Mean={ceiling_tricks.mean():.2f}, Median={np.median(ceiling_tricks):.1f}, Std={ceiling_tricks.std():.2f}")
    print(f"  Bid Amount:  Mean={ceiling_bids.mean():.2f}, Median={np.median(ceiling_bids):.1f}, Std={ceiling_bids.std():.2f}")
    print(f"  Made Bid:    {(ceiling_tricks >= ceiling_bids).sum():,} / {len(ceiling_tricks):,} ({(ceiling_tricks >= ceiling_bids).mean()*100:.1f}%)")
    ...
    print("\n" + "="*80)
    print("TRICK COUNT DISTRIBUTION (Suit Contracts)")
    print("="*80)
    print(f"\n{'Tricks':<10} {'Ceiling':<20} {'CCrider':<20}")
    print("-" * 50)
    for trick_count in range(11):
        ceiling_count = (ceiling_tricks == trick_count).sum()
        ccrider_count = (ccrider_tricks == trick_count).sum()
        ceiling_pct = ceiling_count / len(ceiling_tricks) * 100 if len(ceiling_tricks) > 0 else 0
        ccrider_pct = ccrider_count / len(ccrider_tricks) * 100 if len(ccrider_tricks) > 0 else 0
        print(f"{trick_count:<10} {ceiling_count:>6} ({ceiling_pct:>5.1f}%)    {ccrider_count:>6} ({ccrider_pct:>5.1f}%)")
    
    # Create visualizations
    create_plots(ceiling_tricks, ccrider_tricks, ceiling_bids, ccrider_bids)
    
    print("\n✅ Analysis complete! Plots saved to data/reports/")

def create_plots(ceiling_tricks, ccrider_tricks, ceiling_bids, ccrider_bids):
    ...
    ax1.hist(ceiling_tricks, bins=bins, alpha=0.6, label='OLSa_Ceiling', color='steelblue', edgecolor='black')
    ...
    ax2.hist(ceiling_bids, bins=bins, alpha=0.6, label='OLSa_Ceiling', color='steelblue', edgecolor='black')
    ...
    bp = ax3.boxplot([ceiling_tricks, ccrider_tricks], 
                      tick_labels=['OLSa_Ceiling', 'CCrider'],
                      patch_artist=True,
                      widths=0.5)
    ...
    ax4.scatter(ceiling_bids, ceiling_tricks, alpha=0.3, s=20, label='Ceiling (raw)', color='steelblue')
    ...
    ceiling_binned_means = []
    ccrider_binned_means = []
    bid_values = []
    
    for bid_val in range(5, 11):
        ceiling_mask = ceiling_bids == bid_val
        ccrider_mask = ccrider_bids == bid_val
        
        if ceiling_mask.sum() > 0 or ccrider_mask.sum() > 0:
            bid_values.append(bid_val)
            ceiling_binned_means.append(ceiling_tricks[ceiling_mask].mean() if ceiling_mask.sum() > 0 else np.nan)
            ccrider_binned_means.append(ccrider_tricks[ccrider_mask].mean() if ccrider_mask.sum() > 0 else np.nan)
    
    ax4.plot(bid_values, ceiling_binned_means, 'D-', color='darkblue', linewidth=2.5, 
            markersize=10, markeredgecolor='white', markeredgewidth=1.5, 
            label='Ceiling (binned mean)', zorder=5)
    ...
    ceiling_dist = np.array([(ceiling_tricks == t).sum() / len(ceiling_tricks) * 100 for t in trick_bins])
    ccrider_dist = np.array([(ccrider_tricks == t).sum() / len(ccrider_tricks) * 100 for t in trick_bins])
    
    x = np.arange(11)
    width = 0.35
    ax5.bar(x - width/2, ceiling_dist, width, label='OLSa_Ceiling', color='steelblue', edgecolor='black')
    ...
    ceiling_sorted = np.sort(ceiling_tricks)
    ccrider_sorted = np.sort(ccrider_tricks)
    ceiling_cdf = np.arange(1, len(ceiling_sorted) + 1) / len(ceiling_sorted) * 100
    ccrider_cdf = np.arange(1, len(ccrider_sorted) + 1) / len(ccrider_sorted) * 100
    
    ax6.plot(ceiling_sorted, ceiling_cdf, label='OLSa_Ceiling', color='steelblue', linewidth=2)
    ...
    # Save
    output_path = 'data/reports/ccrider_vs_ceiling_suit_distributions.png'
