...
- [x] `#1121` is resolved with the docs aligned to actual branch-protection truth
- [x] `#1046` is resolved with explicit terminology clarification
- [x] `#1118` is closed as already fixed by `#1122`
- [x] `make check-quiet` passes (5 pre-existing font failures on main, unrelated)

## Outcome

**Branch-protection truth validated:** Only `tests` and `governance` are required
by branch protection. `reviewing-changes` is advisory (not required for merge),
confirmed via `gh api repos/{owner}/{repo}/branches/main/protection`.

**Docs changed (5 files):**

| File | Change |
|------|--------|
| `docs/02_agent/CODEX_GITHUB_REVIEW.md` | Fixed "Yes (branch protection)" → "No (advisory)" in table; updated banner, overview, and framing; added terminology disambiguation note |
| `docs/02_agent/PLATFORM_ENTRY_CHECKLIST.md` | Fixed "merge-relevant gate (branch protection)" to advisory; split into separate classification and branch-protection items |
| `.claude/rules/deferred/60_review_gate.md` | Fixed Purpose column "Post-merge" → "Pre-merge" |
| `plans/sessions/2026-03-15_autonomous-agent-ops-workflow.md` | Added terminology note to three-category model; fixed "merge-relevant review gate" phrasing |
| `plans/agent_ops/0_bootstrap/checkpoints.md` | Clarified substrate description with precise category/branch-protection distinction |

**Issues resolved:**

| Issue | Resolution |
|-------|-----------|
| #1121 | Docs reconciled — all files now agree `reviewing-changes` is advisory |
| #1046 | Terminology note added distinguishing branch-protection "advisory" from ops classification `advisory` category |
| #1118 | Closed — #1122 already removed dead code; `SOURCE_TYPES` gap is cosmetic (constant unused in runtime) |
