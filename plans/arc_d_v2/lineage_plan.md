...
**Outputs:**
- `chart_data/shap_feature_ranking.csv` — per-feature mean |SHAP| ranking for GBT
...
- `charts/comparator_ranking_bars.png`, `charts/tail_risk_panel.png`
...
| Chart | CSV Source | What It Shows | Applicable Models |
|-------|-----------|---------------|-------------------|
| `shap_summary.png` | `chart_data/shap_feature_ranking.csv` | Beeswarm: feature importance with direction, per contract | `gbt_av` |
...
Three dashboard-style composite charts serve as the primary visual review
surface for each rung. Existing standalone charts are supporting assets.

| Dashboard | Panels | CSV Sources |
|-----------|--------|-------------|
| `dashboard_model_eval.png` | R² by contract, MAE by contract, pred vs actual, residual distribution, calibration curve, feature importance / selection path, decision agreement | `tables/model_performance.csv`, `chart_data/predictions.csv`, `chart_data/residuals.csv`, `chart_data/calibration_bins.csv`, `chart_data/feature_importances.csv`, `chart_data/selection_paths.csv`, `chart_data/decision_comparison.csv` |
| `dashboard_health.png` | Outcome distributions, seat balance, contract mix, bid rate / pass rate / make rate, bid-level distribution, bid-type breakdown | `chart_data/outcome_distributions.csv`, `chart_data/seat_balance.csv`, `chart_data/contract_mix.csv`, `chart_data/bid_levels.csv`, `tables/behavior_summary.csv`, `tables/behavior_by_contract.csv` |
| `dashboard_competitive.png` | Comparator ranking bars with CIs, tail risk, H2H delta vs anchor by contract, H2H heatmap, cross-rung progression | `tables/comparator_rankings.csv`, `tables/h2h_delta_matrix.csv`, `tables/cross_rung_deltas.csv` |

Dashboard pages are generated by `generate_rung_charts.py` and are the
first charts embedded in `01_results.md`. They are not yet implemented
(Phase 2 of the reporting suite compaction plan).

#### Canonical `chart_data` Inventory

Every chart that does not render directly from a `tables/*.csv` file must
have its source data in `chart_data/`. The following CSVs constitute the
canonical `chart_data` inventory:

| CSV | Producer | Consumed By |
|-----|----------|-------------|
| `predictions.csv` | `generate_interpretability.py` | `pred_vs_actual.png`, `dashboard_model_eval.png` |
| `residuals.csv` | `generate_interpretability.py` | `residual_distribution.png`, `dashboard_model_eval.png` |
| `calibration_bins.csv` | `generate_interpretability.py` | `calibration_curve.png`, `dashboard_model_eval.png` |
| `selection_paths.csv` | `generate_interpretability.py` | `selection_path.png`, `dashboard_model_eval.png` |
| `decision_comparison.csv` | `generate_interpretability.py` | `decision_agreement.png`, `dashboard_model_eval.png` |
| `disagreement_outcomes.csv` | `generate_interpretability.py` | `disagreement_outcomes.png` |
| `shap_feature_ranking.csv` | `generate_interpretability.py` | `shap_summary.png` |
| `shap_dependence.csv` | `generate_interpretability.py` | `shap_dependence_top5.png` |
| `shap_interactions.csv` | `generate_interpretability.py` | `shap_interactions.png` |
| `feature_importances.csv` | `generate_interpretability.py` | `feature_importance.png`, `dashboard_model_eval.png` |
| `context_feature_usage.csv` | `generate_interpretability.py` | Hypothesis analysis |
| `outcome_distributions.csv` | `generate_rung_charts.py` | `outcome_distributions.png`, `dashboard_health.png` |
| `seat_balance.csv` | `generate_rung_charts.py` | `seat_balance.png`, `dashboard_health.png` |
| `bid_levels.csv` | `generate_rung_charts.py` | `bid_level_distribution.png`, `dashboard_health.png` |
| `contract_mix.csv` | `generate_rung_charts.py` | `contract_mix_bars.png`, `dashboard_health.png` |
| `cross_rung_progression.csv` | `generate_rung_tables.py` | `cross_rung_progression.png`, `dashboard_competitive.png` |

Not all CSVs are present at every rung. When a source is unavailable
(e.g., no GBT artifact for SHAP), the corresponding chart panel degrades
...
- The canonical `chart_data` inventory is defined in §12.12
