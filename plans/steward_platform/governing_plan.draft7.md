# Steward Platform — Governing Plan (Draft 7)

**Date:** 2026-04-23
**Status:** PROPOSED — revision of `governing_plan.draft6.md` incorporating analyst-c's narrow review (427-line critique at `plans/steward_platform/draft6_review_analyst-c.md`, graded draft 6 at A-, recommended PROMOTE-AFTER-G-FIXES) plus Claude Code native-substrate integration (Tier S × system-rework) plus operator directive to simplify relative clock-time tracking from mechanism to intent-only.
**Scope:** Mature the existing Bid-Euchre steward control plane into a full-ambition single-repo platform (self-improving, auditable, durably remembered, token-efficient, actively triaging, replay-testable, agent-readable, native-substrate-aware), prove it on a complete research run, and only then decide what to port and whether a meta-layer earns its slot.
**Supersedes:** `governing_plan.draft6.md` (plus all prior drafts). The prior `agent_ops` governing plan remains historical record for shipped Phase 0-4 work and in-flight Phase 5 work absorbed into Phase 0 below.
...
| B.9 Per-lane custom system prompts via `--system-prompt-file` | #7 | `.claude/system_prompts/<lane>.md` files exist for orchestrator / ops / review / analyst-* / author-* / brws-author-* / flex-* (~8 lane archetypes). Steward-session.sh bootstrap (per Primitive G Setup hook) launches each lane with `--system-prompt-file .claude/system_prompts/<lane>.md`. Rationale: Claude Code default system prompt is known to degrade 4.7+ behavior vs. sparse custom prompts; see `knowledge/harness_assumptions.md` "Default system prompt behavior" entry | Every fleet launch passes `--system-prompt-file`; zero launches fall back to default; observable behavior improvement tracked (e.g., prompt-policy-cited-in-trace rate rises after B.9 lands) | See §11-B row 2 |
...
### 10.9 Extensibility patterns (draft 7, new)
...
**Pattern 2 — Native-substrate-first defaults.** New work checks if a
Claude Code native feature exists before building bespoke. The
`harness_assumptions.md` register tracks entries of the form "we are
bespoke because native doesn't cover X." The changelog review skill
(§5-D) refreshes those entries when native ships a capability; the
refresh proposes a bespoke → native migration when warranted. This is
the discipline that prevents the platform from accumulating bespoke
implementations that native has silently superseded.
...
**Enforcement surface.** Patterns 1-6 are enforced through a combination
...
(Primitive D), and per-primitive readiness criteria that check for
conformance. No pattern is aspirational-only; each has at least one
mechanization path.
...
## 19. Delta From Draft 3 (preserved for lineage)
...
**Surprise finding:** The review → revision cycle across drafts 1→7 (draft 1 → analyst-a B → drafts 2-4 → analyst-b B+ → draft 5 → operator directives → draft 6 → analyst-c A- → draft 7 with Tier S × system-rework + operator additions) is itself a demonstration of the Phase 2 Decision Inputs pattern, the agent-readability theme (goal #16), the hybrid subsections+digest mechanism, the changelog-review-skill pattern (operator-fed external signals integrated into plan revision), AND the extensibility patterns (§10.9) — all working in miniature. Each draft's "Delta From Draft N" section behaves as a structured finding log; analyst review artifacts fed directly into successor draft scope; operator directives layered in clean increments; native-substrate evidence (Tier S list, Boris Cherny effort/recap/focus thread, davidad system-prompt-file thread) accrued as plan-level Tier S adoptions. Worth reflecting in ADR 002 (Open Item #12) as the empirical basis for the durable-infrastructure framing in §15.8 and §10.8.
**Disposition:** open
