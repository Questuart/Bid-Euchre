...
> **Promoted from draft 8 on 2026-04-23.** This file is the canonical governing plan. Lineage: `_archive/governing_plan.draft{1..7}.md` (draft 1 is the operator's original; drafts 2–7 are successive analyst-review revisions; draft 8 was the pre-promotion revision with 12 final tightening fixes applied per `draft8_final_review_handoff.md`). The lineage sections inside this document still refer to prior drafts by name for historical traceability; live references to specific primitives, §-numbers, and sub-deliverables all resolve inside this file.

**Date:** 2026-04-23
**Status:** PROMOTED (canonical) — revision of `governing_plan.draft7.md` (now at `_archive/`) incorporating analyst-d's two-track review (901-line critique at `plans/steward_platform/draft7_review_analyst-d.md`, graded draft 7 at A, recommended PROMOTE-AFTER-FIXES) plus an external-analyst change set against draft 6 applied to draft 7 (goal-1 mechanism expansion; B.11/B.12; improvement-quality metrics; preflight 11th item; overfitting risk; Chrome-native control; core-framing reframe) plus plugin-ecosystem three-tier preference integration plus the draft 8 → canonical tightening pass (12 fixes applied 2026-04-23 per `draft8_final_review_handoff.md`).
...
- **G4 (low-medium) — §15 schema didn't represent the "insufficient evidence" class.** Added 5th prompt to §15.2 subsection schema: `**Re-evaluation needed in Phase 3:** [...]`; §15.4 digest parser extended; §15.7 Phase 2 use handles Re-evaluation via inline `RE-EVAL: <window>` tag across the 4 existing axis buckets (I4 fix supersedes the original "fifth bucket" framing — see §15.7 and §15.4).
...
**Phase 0 path-verification precondition (I6, added in draft 8 tightening).** Before dispatching either audit, verify the checkout path and access model for each target repo and record the actual path if it differs from the default. The audit cannot proceed until this is resolved; "repo not present / access blocked" is a first-class audit outcome (treated as a portability-signal finding and captured directly in the audit report's per-repo section).

- **Target-repo `RIN-SnD` path:** TBD by operator. If the repo is not yet on this host, the audit task packet must either (a) carry a confirmed local checkout path, (b) include access instructions for a remote checkout, or (c) record "repo not present" as the audit outcome with a Phase 2 Decision Inputs subsection stating "not portability-ready at audit time."
- **Target-repo `Fund` path:** previously confirmed present at `/Users/claude_runner/Desktop/Fund` during draft 8 context. Re-verify at Phase 0 kickoff before dispatching the audit task packet (paths can change between planning and execution).
- Access model (read-only vs. read-write needed for audit; tooling prerequisites such as auth tokens) is recorded alongside the path.
...
**Baseline capture mechanization (I7, added in draft 8 tightening).** Baseline capture is tool-backed, not narrative. `scripts/internal/capture_steward_baseline.py` (~40 lines, owned by Primitive G per the Pattern 9 ownership audit) wraps the named capture commands below, writes a timestamped snapshot into `plans/steward_platform/0_hardening/baseline.md`, and serves as the single invocation that Primitive F re-runs whenever a post-reduction re-baseline is triggered (e.g., after Read-tool token reductions land per the draft 7 note at line 515). The skeleton file `plans/steward_platform/0_hardening/baseline.md` is authored at Phase 0 kickoff and contains these sections, each populated by the corresponding command:

| Baseline section | Capture command(s) | Notes |
|---|---|---|
| Attention compression | `uv run python scripts/internal/ops.py dashboard --json \| jq '.lanes[] \| {lane, interventions_last_7d}'` | Operator interventions per task, sampled from last N packets. |
| Token-per-successful-merge | `uv run python scripts/internal/ops.py usage --json --window 30d` + `gh pr list --state merged --limit 50 --json number,title,mergedAt` (joined in script) | Tokens consumed ÷ PRs merged over the window. |
| Issue-discovery share | `gh issue list --state all --limit 200 --json number,title,author,labels,createdAt` filtered in script by `author=app` vs. `author=human` | Share of issues created by automation vs. operator discovery. |
| Message-bus latency | `uv run python scripts/internal/ops.py message stats --json` (p50 / p95 fields) | Telemetry landed in PR #2725. |
| Event-to-operator-signal | `uv run python scripts/internal/ops.py events query --event-type event_emitted --window 7d --json` + recent incident fingerprints (event → `push_delivered` correlation) | Time from event emission to operator awareness. |

If a command fails or returns no data, the baseline entry records "insufficient data at capture time" and feeds a Phase 2 Decision Input under the Re-evaluation tag. No baseline entry is left blank or placeholder-filled.
...
| B.3 Prompt-policy registry | #7 | Registry exists with initial policies for orchestrator/ops/review/analyst-*/author-*; versioned; rollback via version pin. **Registry home:** `.claude/rules/prompt_policy/{orchestrator,author,analyst,common}.md` (landed via PR #2762). | Prompt-policy version cited in ≥90% of proving-run traces | See §11-B row 2 |
...
| B.9a Per-archetype system-prompt files (artifact readiness) | #7 | `.claude/system_prompts/<archetype>.md` files exist for 8 lane archetypes (orchestrator / ops / review / analyst / author / brws-author / flex / scratch). Relationship to existing `.claude/agents/<lane>.md` (23 files currently loaded by Agent tool) **resolved via ADR at Phase 0 kickoff (G10 fix)** — operator picks one of: (a) **Replacement** — system_prompts supplants agents-file body content; agents-file carries frontmatter/metadata only; (b) **Supplement** — system_prompts is sparse override loaded in addition to agents-file; loading order documented; (c) **Orthogonal** — system_prompts covers per-launch `--system-prompt-file` override; agents-file covers Agent-tool-loaded subagent behavior; both persist describing different things. Default assumption until ADR files: (c) orthogonal. Rationale: Claude Code default system prompt degrades 4.7+ behavior vs. sparse custom prompts (harness_assumption "Default system prompt behavior"). **Upstream gate:** G13 (19-lane → 8-archetype mapping) must complete before B.9a prompt files can be authored. | All 8 archetype prompt files committed; G10 ADR filed before any Primitive G adoption work begins; G13 mapping referenced as input to file-authoring | See §11-B row 2 |
| B.9b Fleet launch adoption of `--system-prompt-file` | #7 | Steward-session.sh bootstrap (per Primitive G Setup hook) launches each lane with `--system-prompt-file .claude/system_prompts/<archetype>.md`. **Upstream gate:** B.9a complete (files must exist before launch can reference them); Primitive G Setup-hook scaffolding available. Dependency direction is one-way: G13 → B.9a → B.9b. (No bidirectional readiness; the prior draft's bidirectional formulation was ambiguous.) | Every fleet launch passes `--system-prompt-file`; zero launches fall back to default; observable behavior improvement tracked (e.g., prompt-policy-cited-in-trace rate rises after B.9b lands) | See §11-B row 2 |
...
- **Agent-readability scorecard** (renamed from "legibility scorecard" per goal #16; same ten-row table, primary audience clarified): `knowledge/agent_readability_scorecard.md` measures whether the repo is navigable by agents. Rows: (1) CLAUDE.md ≤ 200 lines; (2) single canonical entry point for new sessions; (3) active governing plan findable in ≤2 hops from repo root; (4) all skills discoverable from `.claude/skills/`; (5) lane registry authoritative not inferred; (6) MEMORY.md indexes rather than recaps; (7) ADR index current; (8) KB INDEX current; (9) no orphan references in plans; (10) rule files grep-discoverable from CLAUDE.md. **Provisional floor: ≥7/10 rows pass.** ADR 001 records the floor; sub-plan under Primitive C may tighten to 8/10 or 9/10 but may not loosen. Re-scored at end of Phase 0 (preflight item 8 sub-criterion) and end of Phase 1.
...
- **Setup hook adoption:** replace imperative `steward-session.sh` with a Setup hook spec + declarative lane config. Pass `--system-prompt-file .claude/system_prompts/<archetype>.md` (B.9b, consuming files produced under B.9a) to every lane launch. Setup-hook adoption consumes pre-existing prompt files; file creation is B.9a under Primitive B. Dependency chain: G13 (19-lane → 8-archetype mapping) → B.9a (authored prompt files) → B.9b (this work bullet, fleet launch adoption).
...
- **`plans/sessions/` archive sweep (G12):** implement `scripts/internal/sweep_session_plans.py` (one-shot; invokable ad hoc) — scans `plans/sessions/` (~264 files as of draft 8), classifies each entry by outcome linkage (merged-PR / abandoned / still-open / missing-outcome), and proposes archival moves into `plans/sessions/_archive/<year>/`. Operator reviews the proposal before the sweep executes. Pattern 9 ownership: script home is Primitive G; loaded by `/sweep-session-plans` skill. See §10.9 Pattern 9.
- **Baseline capture script ownership (I7):** `scripts/internal/capture_steward_baseline.py` (~40 lines) captures the Phase 0 baseline for Primitive F signal collection — see §4.3 for the command list and output schema. Script home is Primitive G (baseline capture is operational tooling that Primitive F consumes).
...
- Setup hook replaces `steward-session.sh`: declarative lane config + Setup-hook-driven launch; every lane launch passes `--system-prompt-file .claude/system_prompts/<archetype>.md` (B.9b).
- `.claude/system_prompts/<archetype>.md` files (B.9a) exist for 8 lane archetypes; sparse by default per the "default system prompt degrades 4.7+" harness assumption. **Readiness dependency note:** Primitive G Setup-hook adoption (B.9b) consumes these pre-existing prompt files authored under Primitive B (B.9a). The chain is strictly one-way: G13 (19 → 8 archetype mapping, first-deliverable sub-sub-plan under Primitive G) → B.9a (file creation under Primitive B) → B.9b (fleet launch adoption under Primitive G). No bidirectional readiness exists between B and G for this surface.
...
- `scripts/internal/sweep_session_plans.py` committed and the `plans/sessions/` archive sweep has been run at least once (operator approved the proposal; files moved under `plans/sessions/_archive/<year>/`; `plans/sessions/` inflight count drops from ~264 to the active-work subset). Script is owned by Primitive G per §10.9 Pattern 9.
- `scripts/internal/capture_steward_baseline.py` committed; one seed baseline captured to confirm the script runs (Primitive F consumes it during proving-run signal collection; full baseline snapshot lives under Primitive F).
...
- Pass/fail status for each of the 11 checklist items.
- Any gaps identified, with Phase 0 follow-ups if re-opening is required.
- **Phase 2 Decision Inputs subsection** (§15) for each of the 11 checklist items.

### 6.6 Preflight gate

- **All 11 pass:** proceed to Phase 1 (after data-sufficiency check §6.1).
...
| Preflight passes trivially without exercising surfaces | Preflight pass/fail checklist (§6.4) is granular (11 items across all primitives); selection criterion (§6.3) forces cross-lane exercise |
...
2. Phase 1a preflight passes all 11 checklist items or completes the re-work loop per §6.6.
...
11. **ADR 001 content** *(authored at Phase 0 kickoff, not pre-promotion; evidence will be supplied from Phase 0 observation).* Draft at Phase 0 kickoff; names three-to-four concrete constraints that paused Platform-11/13 and have since lifted (see §12 Risks row 6); also records the agent-readability scorecard floor (≥7/10) per F10. Tag added in draft 8 tightening (orchestrator omission Orch-1) to clarify that the ADR body is a Phase 0 kickoff deliverable, not a pre-promotion gate. The governing plan may promote to canonical with ADR 001 still unfiled; the kickoff task packet will open a sub-plan to fill it.
...
16. **ADR 006 — Auto mode codification** (Primitive G/B) *(authored at Phase 0 kickoff, not pre-promotion; evidence will be supplied from Phase 0 observation).* Documents the user-scope `autoMode.environment` configuration the fleet currently runs under (per `.claude/rules/80_permission_model.md`); records the trust-envelope content; specifies refresh cadence. Tag added in draft 8 tightening (orchestrator omission Orch-1) for the same reason as ADR 001: the ADR body is a Phase 0 kickoff deliverable, not a pre-promotion gate.
...
- Parses each subsection's 5 prompts + disposition status + source file path.
- Groups entries into 4 decision-axis buckets (portability / meta-layer / kill / surprise), with any one entry possibly appearing in multiple buckets. **Re-evaluation entries are tagged inline with a `RE-EVAL: <window>` marker rather than placed in a separate bucket. Rationale: Re-eval is a temporal confidence property (when to look again), not a decision axis (what decision to make); placing it in a separate bucket would force Phase 2 reviewers to deduplicate across the decision axes it actually informs. Tag-based extraction preserves the 4-axis structure and lets the same entry carry a re-evaluation window alongside its decision-axis disposition.** Model choice recorded under `I4 design decision` — Model B (4 axis buckets + inline `RE-EVAL` tag) chosen over Model A (5th bucket).
...
- **Re-evaluation handling (tag-based extraction, draft 8 I4 fix — supersedes the draft 7 G4 "fifth bucket" framing):** the 5th prompt (`Re-evaluation needed in Phase 3`) is not a separate decision axis; it is a temporal confidence tag that may apply to any axis. The digest parser extracts a `RE-EVAL: <window>` tag from each subsection with a non-"no" 5th-prompt value and attaches the tag to the entry in every axis bucket where that entry appears. Phase 2 consumes tagged entries alongside their axis-native decisions (e.g., a portability-readiness entry tagged `RE-EVAL: 6mo` contributes to both the portability decision now and the Phase 3 re-evaluation queue). This keeps the 4-axis decision surface clean while preserving the "insufficient evidence" signal introduced by the §11 soft re-evaluation trigger.
...
- B sub-deliverables: B.1–B.12 with B.9 split into B.9a (artifact readiness) + B.9b (fleet launch adoption) — 13 rows total across 12 slot labels (was B.1–B.10 in draft 7; draft 8 added B.11 orchestration recipe + B.12 improvement-mechanism evaluation; the B.9a / B.9b split was introduced during the draft-8 pre-promotion tightening pass per finding I1)
...
- **Legibility scorecard renamed to "agent-readability scorecard"** (operator directive driving goal #16 application). Same 10 items (pre-draft 8; draft 8 preflight grew 10 → 11 via the repeat-task improvement probe, but the scorecard itself remains 10-item); primary audience clarified.
...
- **Preflight checklist** (§6.4) swaps the replay item (Primitive H, not available at preflight time) for the agent legibility item (Primitive C). Preflight was a 10-item gate at this change (pre-draft 8; expanded to 11 items in draft 8 per the external-analyst change set).
