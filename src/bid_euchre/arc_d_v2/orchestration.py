...
def _collect_training_artifacts(
    ...
    for model in roster.trainable_models():
        ...
        # Copy companion files (.joblib for GBT models)
...
def execute_step_5(state: RunState, seed: int, dry_run: bool = False) -> bool:
    ...
    # Find the batch manifest generated by the comparator for --single-seat mode
    experiment_name = comparator_config.get("experiment_name", "")
    manifest_glob = f"batch_manifest_{experiment_name}_{seed}_*.json"
    manifests = sorted(runs_dir.glob(manifest_glob))
    ...
    if manifest_path:
        cmd_cis.extend(["--manifest", str(manifest_path)])
