...
## 10. Auction Mode Follow-Up (2026-02-04)

### 10.1 Motivation

The bidless experiments (Section 4) showed F1+F4 significantly outperforms F1+F4+F5
(+0.056 tricks, p=0.035). This follow-up tests whether that advantage persists
when actual bidding selects contracts.

### 10.2 Experimental Setup

| Parameter | Value |
|-----------|-------|
| Mode | head_to_head (auction) |
| Bidding Policy | StrictRaiserBidder (both teams) |
| Hands | 50,000 |
| Seed | 42 |
| Paired Deals | Yes |
...
| Mode | Strategy A | Strategy B | N | Δ Tricks (A-B) | 95% CI | p-value | Cohen's d | Winner |
|------|------------|------------|---|----------------|--------|---------|-----------|--------|
| Bidless | F1+F4 | F1+F4+F5 | 100K | +0.056 | [+0.004, +0.108] | **0.035** | 0.018 | **F1+F4** |
| Auction | F1+F4 | F1+F4+F5 | 50K | -0.020 | [-0.055, +0.015] | 0.267 | -0.005 | None |

#### Auction Mode Win Rates

| Metric | F1+F4 | F1+F4+F5 |
|--------|-------|----------|
| Mean Tricks | 4.9901 | 5.0099 |
| Hands Won | 19,962 (39.9%) | 20,077 (40.2%) |
| Ties | 9,961 (19.9%) | — |

### 10.4 Conclusions

1. **Bidding neutralizes the F1+F4 advantage** - The +0.056 trick advantage in bidless mode
   disappears when bidding is involved (p=0.267, not significant).

2. **F5 (sure_winner_cover) is context-dependent** - Hurts in forced contracts, neutral in
   self-selected contracts.

3. **Recommendation unchanged** - For frozen play strategy, F1+F4 remains the default
   (simpler, equivalent performance in auction mode, better in bidless).
