# Phase 0 Bidless Report (r2) — 2026-02-07

> **File Goal:** Immutable record of Phase 0 bidless data foundation status.
> **Work Goal:** Confirm simulation health, freeze glutton as play policy, validate feature signal for downstream bidding.
> **Snapshot date:** 2026-02-07
> **Snapshot ID:** `phase0_20260207_r2`
...
> **Provenance:** [`phase0_bidless_20260207_r2_provenance.json`](phase0_bidless_20260207_r2_provenance.json)

### Key Takeaways

1. **Glutton frozen** as canonical play policy — PASS gate with +0.19 to +0.21 mean trick advantage; all 95% bootstrap CIs exclude zero; one-sample t-test confirms (p < 0.001)
2. **All sanity gates pass** — zero FAIL counts across all canonical runs
3. **Diagnostic Ridge R² ~ 0.19–0.21** on tricks_won — confirms hand features carry predictive signal for downstream bidding

---

## 1. Run Health Summary

Before examining results, these checks confirm the simulation infrastructure is sound.

### 1.1 Self-Play Fairness
...
## 2. Strategy Sanity Tests (Zoom Run)
...
## 3. Play Policy Gate: Glutton > Greedy

> Source: `play_policy_gate_aggregate_20260204_221656.json`
...
- **Statistics:**
  - Bootstrap 95% CI on mean glutton advantage (positive = glutton better)
  - One-sample t-test: H0: mean advantage = 0
- **Gate criterion:** PASS if all bootstrap CIs exclude zero
...
**All 6 CIs exclude zero.** Lowest CI lower bound: +0.1639 (seed 42, greedy_vs_glutton). One-sample t-test confirms significance (p < 0.001 for all directions).

### 3.3 Per-Scenario Breakdown (Seed 42)
...
**Glutton frozen as canonical play policy.** LOW's marginal significance is documented as a known caveat but does not affect the pooled gate decision.

---

## 4. Diagnostic Feature Evaluation
...
### 4.3 Top-10 Standardized Coefficients
...
> Source: 5 canonical runs + 1 policy gate, all produced 2026-02-04 at git SHA `ea55269`
...
- **`bidless_outcomes.parquet`** — Per-hand rows: `hand_id`, `tricks_team0`, `tricks_team1`, `team0_win`
