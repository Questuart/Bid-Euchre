...
| `scripts/analyze_bower_validation.py` | Glutton bower validation experiment analysis (bootstrap CIs) |
...
| `scripts/internal/triage_cli.py` | Programmatic triage CLI (SCAFFOLD — Primitive E Phase 0 Packet E1); provides `TriageInput`, `TriageResult`, `file_or_recur()` surface consumed by the `triaging-issues` skill; live runtime blocked on Primitive A Packet 3 |
