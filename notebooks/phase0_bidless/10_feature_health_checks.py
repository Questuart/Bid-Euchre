...
def plot_violin_box(data, x, y, ax, palette="Set2", box_width=0.15, **kwargs):
    ...
    try:
        ...
        sns.violinplot(
            data=data, x=x, y=y, ax=ax, inner=None, cut=0, palette=palette, **kwargs
        )
        sns.boxplot(
            data=data,
            x=x,
...
def is_paired_data(
    df: pd.DataFrame, group_col: str, value_col: str = "hand_id"
) -> bool:
...
contract_stats = df.groupby("contract_type")["feat_hand_value"].agg(
...
contract_groups = [
    df[df["contract_type"] == ct]["feat_hand_value"].values
...
print(
    f"  Interpretation: {eta_squared*100:.2f}% of variance explained by contract_type"
)
...
if len(suit_df) > 0:
    ...
    trump_stats = suit_df.groupby("trump_suit")["feat_hand_value"].agg(
        ["count", "mean", "std"]
    )
    display(trump_stats)
    ...
    groups = [
        suit_df[suit_df["trump_suit"] == suit]["feat_hand_value"].values
        for suit in ["C", "D", "H", "S"]
    ]
    ...
    print(
        f"  {'Chi-squared' if test_name == 'Friedman' else 'F'}-statistic: {stat:.4f}"
    )
    print(f"  p-value: {p_value:.4f}")
    print("  Significance level: α = 0.05")
    print(f"  Test type: {test_name} ({'paired' if paired else 'independent'})")
    ...
    ss_between = sum(
        len(suit_df[suit_df["trump_suit"] == suit])
        * (
            suit_df[suit_df["trump_suit"] == suit]["feat_hand_value"].mean()
            - grand_mean
        )
        ** 2
        for suit in ["C", "D", "H", "S"]
    ...
    print(f"  Effect size (η²): {eta_squared:.4f}")
    print(
        f"  Interpretation: {eta_squared*100:.2f}% of variance explained by trump suit"
    )
    ...
    if p_value < 0.05:
        ...
        for i in range(len(suits)):
            for j in range(i + 1, len(suits)):
                ...
                print(
                    f"    {suits[i]} vs {suits[j]}: p={res.pvalue[i,j]:.4f} {sig_marker}"
                )
    else:
...
if len(highlow_df) > 0:
    ...
    hl_stats = highlow_df.groupby("contract_type")["feat_hand_value"].agg(
        ["count", "mean", "std"]
    )
    display(hl_stats)

    # Two-sample t-test
    high_vals = highlow_df[highlow_df["contract_type"] == "high"][
        "feat_hand_value"
    ].values
    low_vals = highlow_df[highlow_df["contract_type"] == "low"][
        "feat_hand_value"
    ].values
    ...
    if p_value < 0.05:
        print(
            f"  ❌ FAIL: Significant difference between high and low (p={p_value:.4f})"
        )
    else:
    ...
    plot_violin_box(
        highlow_df, x="contract_type", y="feat_hand_value", ax=ax, palette="Set2"
    )
...
if len(suit_contracts) > 0:
    plot_violin_box(
        suit_contracts, x="team", y="feat_hand_value", ax=axes[0], palette="Blues"
    )
...
if len(highlow_contracts) > 0:
    plot_violin_box(
        highlow_contracts, x="team", y="feat_hand_value", ax=axes[1], palette="Greens"
    )
...
if len(suit_contracts) > 0:
    ...
    suit_team_stats = suit_contracts.groupby("team")["feat_hand_value"].agg(
        ["count", "mean", "std"]
    )
...
if len(highlow_contracts) > 0:
    ...
    highlow_team_stats = highlow_contracts.groupby("team")["feat_hand_value"].agg(
        ["count", "mean", "std"]
    )
    display(highlow_team_stats)

    team0_hl = highlow_contracts[highlow_contracts["team"] == 0][
        "feat_hand_value"
    ].values
    team1_hl = highlow_contracts[highlow_contracts["team"] == 1][
        "feat_hand_value"
    ].values
...
if len(suit_contracts) > 0:
    plot_violin_box(
        suit_contracts, x="seat", y="feat_hand_value", ax=axes[0], palette="Blues"
    )
...
if len(highlow_contracts) > 0:
    plot_violin_box(
        highlow_contracts, x="seat", y="feat_hand_value", ax=axes[1], palette="Greens"
    )
...
if len(suit_contracts) > 0:
    ...
    suit_seat_stats = suit_contracts.groupby("seat")["feat_hand_value"].agg(
        ["count", "mean", "std"]
    )
    display(suit_seat_stats)

    suit_groups = [
        suit_contracts[suit_contracts["seat"] == seat]["feat_hand_value"].values
        for seat in range(4)
    ]
    ...
    ss_between = sum(
        len(suit_contracts[suit_contracts["seat"] == seat])
        * (
            suit_contracts[suit_contracts["seat"] == seat]["feat_hand_value"].mean()
            - grand_mean
        )
        ** 2
        for seat in range(4)
...
if len(highlow_contracts) > 0:
    ...
    highlow_seat_stats = highlow_contracts.groupby("seat")["feat_hand_value"].agg(
        ["count", "mean", "std"]
    )
    display(highlow_seat_stats)

    highlow_groups = [
        highlow_contracts[highlow_contracts["seat"] == seat]["feat_hand_value"].values
        for seat in range(4)
    ]
    ...
    ss_between = sum(
        len(highlow_contracts[highlow_contracts["seat"] == seat])
        * (
            highlow_contracts[highlow_contracts["seat"] == seat][
                "feat_hand_value"
            ].mean()
            - grand_mean
        )
...
print(
    f"  Max deviation: {balance.max_deviation:.4f} (seat {balance.max_deviation_seat})"
)
...
try:
    ...
    print(
        "\n  Main effect (Contract_Type): "
        + ("❌ SIGNIFICANT" if ct_p < 0.05 else "✅ Not significant")
    )
    print(
        "  Main effect (Team): "
        + ("❌ SIGNIFICANT" if team_p < 0.05 else "✅ Not significant")
    )
    print(
        "  Interaction: "
        + ("❌ SIGNIFICANT" if interaction_p < 0.05 else "✅ Not significant")
    )
    ...
    for idx, feat_name in enumerate(features_to_plot):
        ...
        for val in feat_values:
            for suit in ["C", "D", "H", "S"]:
                count = len(
                    suit_df[
                        (suit_df[col_name] == val) & (suit_df["trump_suit"] == suit)
                    ]
                )
        ...
        if (
            ...
            ax.set_xticklabels(
                [
                    f"{feat_values[i]:.1f}"
                    if isinstance(feat_values[i], float)
                    else str(feat_values[i])
            ...
            ax.set_xticks(x)
            ax.set_xticklabels(
                [f"{v:.1f}" if isinstance(v, float) else str(v) for v in feat_values],
                rotation=45,
            )
...
if len(highlow_df) > 0:
    ...
    for idx, feat_name in enumerate(highlow_features):
        ...
        if len(feat_values) > 20:
            ...
            bins = np.linspace(
                highlow_df[col_name].min(), highlow_df[col_name].max(), 15
            )
        ...
        ax.set_xticklabels(
            [
                str(feat_values[i])[:6]
                for i in range(0, len(feat_values), max(1, len(x) // 10))
            ],
            rotation=45,
        )
        if idx == 0:
...
if len(highlow_df) > 0:
    ...
    highlow_feature_cols = [
        "feat_high_card_count",
        "feat_low_card_count",
        "feat_double_ten_jack_count",
        "feat_offsuit_aces",
    ...
    # Filter to only features that exist
    highlow_feature_cols = [
        col for col in highlow_feature_cols if col in highlow_df.columns
    ]
...
highlow_features = [
    c
    for c in feat_cols
    if any(x in c for x in ["high_card", "low_card", "double_ten_jack"])
]
...
if len(suit_df) > 0:
    suit_features = [
        c for c in trump_features + general_features if c in suit_df.columns
    ]
...
if len(highlow_df) > 0:
    hl_features = [
        c for c in highlow_features + general_features if c in highlow_df.columns
    ]
...
if len(general_features) > 0:
    ...
    if len(top_general_features) > 0:
        ...
        try:
            ...
            im = axes[2].imshow(
                corr_general, cmap="RdBu_r", vmin=-1, vmax=1, aspect="auto"
            )
            axes[2].set_xticks(range(len(top_general_features)))
            axes[2].set_yticks(range(len(top_general_features)))
            axes[2].set_xticklabels(
                [c.replace("feat_", "") for c in top_general_features], rotation=90
            )
            axes[2].set_yticklabels(
                [c.replace("feat_", "") for c in top_general_features]
            )
...
if len(suit_df) > 0 and "feat_hand_value" in suit_df.columns:
    suit_feat_cols = [
        c for c in suit_df.columns if c.startswith("feat_") and c != "feat_hand_value"
    ]
...
if len(highlow_df) > 0 and "feat_hand_value" in highlow_df.columns:
    hl_feat_cols = [
        c
        for c in highlow_df.columns
        if c.startswith("feat_") and c != "feat_hand_value"
    ]
...
if "feat_hand_value" in df.columns:
    all_feat_cols = [
        c for c in df.columns if c.startswith("feat_") and c != "feat_hand_value"
    ]
...
if (
    len(suit_df) > 0
    and "feat_trump_count" in suit_df.columns
    and "feat_hand_value" in suit_df.columns
):
    suit_colors = {"C": "tab:blue", "D": "tab:orange", "H": "tab:red", "S": "tab:green"}
    ...
    for suit, color in suit_colors.items():
        ...
        if len(subset) > 0:
            ax.scatter(
                subset["feat_trump_count"],
                subset["feat_hand_value"],
                c=color,
                alpha=0.4,
                label=suit,
                s=20,
...
if (
    len(highlow_df) > 0
    and "feat_high_card_count" in highlow_df.columns
    and "feat_hand_value" in highlow_df.columns
):
    contract_colors = {"high": "tab:green", "low": "tab:purple"}
...
if (
    len(suit_df) > 0
    and "feat_bowers" in suit_df.columns
    and "feat_hand_value" in suit_df.columns
):
    suit_colors = {"C": "tab:blue", "D": "tab:orange", "H": "tab:red", "S": "tab:green"}
...
if "feat_offsuit_aces" in df.columns and "feat_hand_value" in df.columns:
    ...
    for ct, color in contract_colors.items():
        ...
        if len(subset) > 0:
            ...
            jitter = np.random.normal(0, 0.05, len(subset))
            ax.scatter(
                subset["feat_offsuit_aces"] + jitter,
                subset["feat_hand_value"],
                c=color,
                alpha=0.3,
                label=ct,
...
print("\nExpected patterns:")
print(
    "- Suit contracts: Positive correlation between trump_count/bowers and hand_value"
)
...
if len(suit_df) > 0 and "feat_hand_value" in suit_df.columns:
    ...
    ax.set_title(
        f"Rolling Mean: Suit Contracts by Trump Suit\n(window={ROLLING_WINDOW})"
    )
...
if "feat_hand_value" in df.columns:
    ...
    ax.text(
        0.5, 0.5, "No hand_value data", ha="center", va="center", transform=ax.transAxes
    )
...
if len(suit_df) > 0 and "feat_hand_value" in suit_df.columns:
    ...
    suit_df["decile"] = pd.Series(
        (suit_df_indexed.index // decile_size_suit), index=suit_df.index
    ).clip(0, 9)

    # Compute mean hand_value for each (decile, trump_suit) pair
    suit_decile_stats = (
        suit_df.groupby(["decile", "trump_suit"])["feat_hand_value"].mean().unstack()
    )
    ...
    for suit in ["C", "D", "H", "S"]:
        ...
        print(
            f"  Trump {suit}:  slope={slope:+.4f}  p={p_value:.4f}  R²={r_value**2:.4f}  {status}"
        )
...
if len(highlow_df) > 0 and "feat_hand_value" in highlow_df.columns:
    ...
    highlow_df["decile"] = pd.Series(
        (highlow_df_indexed.index // decile_size_highlow), index=highlow_df.index
    ).clip(0, 9)

    # Compute mean hand_value for each (decile, contract_type) pair
    highlow_decile_stats = (
        highlow_df.groupby(["decile", "contract_type"])["feat_hand_value"]
        .mean()
        .unstack()
    )
    ...
    for ct in ["high", "low"]:
        ...
        print(
            f"  {ct.capitalize():5s}:  slope={slope:+.4f}  p={p_value:.4f}  R²={r_value**2:.4f}  {status}"
        )
...
if "feat_hand_value" in df.columns:
    ...
    overall_decile_stats = df.groupby("decile")["feat_hand_value"].agg(
        ["mean", "std", "count"]
    )
    ...
    for suit, color in suit_colors.items():
        ...
        if len(subset) > 0:
            ...
            try:
                ...
                sns.boxplot(
                    data=subset, x="decile", y="feat_hand_value", ax=ax, color=color
                )
            except ImportError:
