...
| P16 | Implementation handoffs require disciplined execution sequence | If the plan is handing execution to another agent, verify it explicitly requires: plan refresh/draft, spawned plan review, task list creation, parallelism assessment, and end-to-end autonomous execution/PR shipment | Repo handoff protocol |
...
- **P16 applies only to implementation handoffs/execution directives.** SKIP for pure research notes, reflective writeups, or plans that are not directing another agent to execute work.
